...
def parse_patch(patch: str) -> list[ParsedLine]:
    ...
    for raw in patch.splitlines():
        ...
        if raw.startswith("+"):
            ...
            result.append(ParsedLine(new_line=new_line, old_line=old_line, content=raw[1:] if raw.startswith(" ") else raw, kind="context"))
            old_line += 1
            new_line += 1
...
def get_file_patches(pr_files: list[dict]) -> dict[str, dict]:
    ...
    for f in pr_files:
        ...
        if "patch" not in f:
            ...
            out[path] = {"skipped": True, "reason": "no patch (binary or too large)", "renamed_from": f.get("previous_filename")}
            continue
        out[path] = {
            "skipped": False,
